...
-- DROP existing tables first (in reverse dependency order) to ensure clean schema
DROP TABLE IF EXISTS journal_entries CASCADE;
DROP TABLE IF EXISTS users CASCADE;
DROP TABLE IF EXISTS couples CASCADE;
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;

-- 1. Create couples table (UUID for table id, TEXT for user ids from auth.uid())
...
-- 2. Create users table (TEXT for user id to match auth.uid() - NO FK to auth.users)
CREATE TABLE users (
  id TEXT PRIMARY KEY,
...
-- 12. Verify tables created successfully
